...
bool Server::InitDB()
{
     ...
     SetConsoleTitle("||| 35%");
     Log(LOAD,true,"[|||||||             ] 35% > Criando handle MYSQL...");
     do {
         mysql = mysql_init(0);
         if (!mysql) {
            Log(ERRO,true,"Erro ao criar a handle do MYSQL! %s", mysql_error(mysql));
            if (MessageBox(NULL, "Erro ao criar a handle do MYSQL!", "MYSQL", MB_RETRYCANCEL | MB_ICONWARNING) == IDCANCEL) {
                CloseDB();
                return false;
            }
         } else {
     ...
     SetConsoleTitle("|||| 40%");
     Log(LOAD,true,"[||||||||            ] 40% > Conectando ao MYSQL...");
     do {
         if (!mysql_real_connect(mysql, host, user, pass, db, port, NULL, 0)) {
            Log(ERRO,true,"Erro ao conectar ao MYSQL! %s", mysql_error(mysql));
            if (MessageBox(NULL, "Erro ao conectar ao MYSQL!", "MYSQL", MB_RETRYCANCEL | MB_ICONWARNING) == IDCANCEL) {
                CloseDB();
                return false;
            }
            mysql = mysql_init(0);
     ...
     SetConsoleTitle("|||| 45%");
     Log(LOAD,true,"[|||||||||           ] 45% > Setando timeout do MYSQL...");
     bool continuar = false;
     do {
         if (!QueryDB("SET SESSION interactive_timeout=%i", 2678400)) {
            Log(ERRO,true,"Erro ao setar timeout do MYSQL!");
            switch (MessageBox(NULL, "Erro ao setar timeout do MYSQL!", "MYSQL", MB_ABORTRETRYIGNORE | MB_ICONWARNING)) {
                case IDABORT:
                    CloseDB();
                    return false;
                    break;
     ...
     Log(SYSTEM,true,"Server conectado ao MYSQL v.:%i.", mysql_get_server_version(mysql));
...
bool Server::CloseDB()
{
     Log(SYSTEM,false,"Finalizando conexão MYSQL...");
     mysql_close(mysql);
     return true;
}
...
bool Server::QueryDB(char* Querycmd, ...)
{
     va_list ap;
     va_start(ap, Querycmd);
     char Query[1024];
     vsprintf(Query, Querycmd, ap);
     va_end(ap);
     pthread_mutex_lock(&MutexSQL);
     int erro_sql;
     erro_sql = mysql_query(mysql, Query);
     if (erro_sql != 0) {
        Log(ERRO_SQL, false, "Erro na sintaxe da query MYSQL! %s", mysql_error(mysql));
...
MYSQL_RES* Server::QueryResult(char* Querycmd, ...)
{
     va_list ap;
     va_start(ap, Querycmd);
     char Query[1024];
     ...
     if (outrocliente == NULL) {
        ...
        if (mysql_num_rows(res) > 0) {
           ...
           if (!QueryDB("UPDATE users SET ip ='%s' WHERE id ='%i'", estecliente->ip, estecliente->id)) {
              Log(ERRO,false,"Não foi possível atualizar o ip do player: %i:%s para: %s", estecliente->id, estecliente->usuario, estecliente->ip);
           }

           if (!QueryDB("UPDATE users SET online ='1' WHERE id ='%i'", estecliente->id)) {
              Log(ERRO,false,"Não foi possível atualizar o status do player: %i:%s para online", estecliente->id, estecliente->usuario);
           }

           if (estecliente->ban == 1) {
              if (strcmp(estecliente->lang,"pt_BR")==0) {
                 SendMessage(estecliente,"Esta conta está banida");
           ...
           if (strcmp(estecliente->lang,"pt_BR")==0) {
              SendMessage(estecliente,"Você foi logado com sucesso com a conta: %i:%s no server %s!", estecliente->id, estecliente->usuario, SERVER_NAME);
           } else if (strcmp(estecliente->lang,"en_US")==0) {
           ...
           SendMessage(estecliente, "Esta conta não existe");
           DesconectaPlayer(estecliente);
           return false;
        }
     } else {
        if (strcmp(outrocliente->lang,"pt_BR")==0) {
           SendMessage(estecliente,"Esta conta já está logada");
...
bool Server::CharCreate(player_base* estecliente)
{
    return true;
}

bool Server::CharDelete(player_base* estecliente)
{
    return true;
}
